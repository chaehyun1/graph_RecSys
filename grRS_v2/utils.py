...
def filter(P_bar_user, P_bar_group, user_alpha, group_alpha, filter_pair):
    if filter_pair == 'filter_1D_1D':
        ...
        new_P = user_alpha*P_bar_user + group_alpha *(P_bar_group + 0.1*((-1)*(P_bar_group)**3 + 10*(P_bar_group)**2 + (-29)*P_bar_group))
        return new_P
        ...
        new_P = user_alpha*(2*P_bar_user - (P_bar_user)**2) + group_alpha*(P_bar_group + 0.1*((-1)*(P_bar_group)**3 + 10*(P_bar_group)**2 + (-29)*P_bar_group))
        return new_P

    elif filter_pair == 'filter_3D_1D':
        new_P = user_alpha*(P_bar_user + 0.1*((-1)*(P_bar_user)**3 + 10*(P_bar_user)**2 + (-29)*P_bar_user)) + group_alpha*P_bar_group
        return new_P

    elif filter_pair == 'filter_3D_2D':
        new_P = user_alpha*(P_bar_user + 0.1*((-1)*(P_bar_user)**3 + 10*(P_bar_user)**2 + (-29)*P_bar_user)) + group_alpha*(2*P_bar_group - (P_bar_group)**2)
        return new_P

    elif filter_pair == 'filter_3D_3D':
        new_P = user_alpha*(P_bar_user + 0.1*((-1)*(P_bar_user)**3 + 10*(P_bar_user)**2 + (-29)*P_bar_user)) + group_alpha*(P_bar_group + (-1)*(P_bar_group)**3 + 10*(P_bar_group)**2 + (-29)*P_bar_group)
        return new_P
...
def recall_at_k(gt_mat, results, negative_list, k=10):
    recall_sum = 0
    for i in range(gt_mat.shape[0]):
        relevant_items = set(np.where(gt_mat[i, :] > 0)[0])
        candidates = list(set(relevant_items).union(set(negative_list[i])))
        candidates = [item for item in candidates if item < results.shape[1]]  # 유효한 인덱스만 남김
...
def inference_new(P_user, P_group, ps, version=0, device="cpu"): # cri: criteria, ps: power
    # MCEG_c_user = MCEG_user[n_users * cri : n_users * (cri + 1), :]
    # MCEG_c_group = MCEG_group[n_groups * cri : n_groups * (cri + 1), :]
    P_user = P_user.to_dense()
    P_copy_user = copy.deepcopy(P_user)
    P_group = P_group.to_dense()
    P_copy_group = copy.deepcopy(P_group)
    # linear
    if version == 0:
        P_copy_user.data **= ps
        P_copy_group.data **= ps
        S = P_copy_user + 0.5*P_copy_group
    # # Concave up
    # elif version == 1:
    #     P_copy.data **= ps[1]
...
def normalize_sparse_adjacency_matrix(adj_matrix, alpha): #adj_matrix: [290, 7057]
    # Calculate rowsum and columnsum using COO format
    rowsum = torch.sparse.mm(
        adj_matrix, torch.ones((adj_matrix.shape[1], 1), device=adj_matrix.device)
    ).squeeze() # adj_matrix의 row별 합을 구함: D_U or D_G
    rowsum = torch.pow(rowsum, -alpha)  # 값이 작아짐 
    colsum = torch.sparse.mm(
        adj_matrix.t(), torch.ones((adj_matrix.shape[0], 1), device=adj_matrix.device)
    ).squeeze() # D_I
    ...
    # indices 변수에는 COO 형식의 희소 행렬의 인덱스를 나타내는 텐서가 저장됩니다. 
    # 이 텐서는 (N, 2) 모양을 가지며, 각 행은 희소 행렬의 한 항목을 나타내는 (행 인덱스, 열 인덱스)를 나타냅니다.
    indices = (
        torch.arange(0, rowsum.size(0)).unsqueeze(1).repeat(1, 2).to(adj_matrix.device)
    )
    # d_mat_rows = torch.sparse.FloatTensor(
