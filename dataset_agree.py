...
class DatasetAgree:
    ...
    def datasetToDataFrame(self, file_train, user=True):
        ...
        if user:
            ...
            df = pd.DataFrame(data, columns=['group_id', 'item_id'])
        df['rating'] = 1
        return df 
    ...
    def getDataset(self):
        ...
        unique_group_item = df_group_test['item_id'].unique()
        unique_user_item = df_user_test['item_id'].unique()
        group_not_in_user = set(unique_group_item) - set(unique_user_item)
        user_not_in_group = set(unique_user_item) - set(unique_group_item)
        df_group_test = df_group_test[~df_group_test['item_id'].isin(group_not_in_user)]
        df_user_test = df_user_test[~df_user_test['item_id'].isin(user_not_in_group)]

        # sort: unique groups, items, users
        train_groups = df_group_train['group_id'].unique()
        train_group_items = df_group_train['item_id'].unique()
        test_groups = df_group_test['group_id'].unique()
        test_group_items = df_group_test['item_id'].unique()
        train_users = df_user_train['user_id'].unique()
        train_user_items = df_user_train['item_id'].unique()
        test_users = df_user_test['user_id'].unique()
        test_user_items = df_user_test['item_id'].unique()
        ...
        test_group_to_index = {group: index for index, group in enumerate(test_groups)}
        ...
        train_group_num_items = len(train_group_items) # 7686 -> 7057
        ...
        for _, row in df_group_test.iterrows():
            group_index = test_group_to_index[row['group_id']]
        ...
        #train_group_result = self.scale_sparse_tensor(train_group_result)
        #test_group_result = self.scale_sparse_tensor(test_group_result)
        #train_user_result = self.scale_sparse_tensor(train_user_result)
        #test_user_result = self.scale_sparse_tensor(test_user_result)
